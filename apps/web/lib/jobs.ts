 ...
 *
 * The dashboard deliberately does not reimplement enrich/score/places — the
 * logic lives once in `@leads/core` and both front ends drive it. Spawning
 * rather than calling it in-process is deliberate too: a 30-minute load inside
 * `next dev` dies on the next file save. This module only starts the CLI,
 * captures its output and records the outcome.
 */

// Next runs with cwd = <repo>/apps/web; the CLI needs the repo root so its
// dotenv call finds the root .env (GOOGLE_MAPS_API_KEY, OPEN_ROUTER_API_KEY
// live there — apps/web/.env.local only has DATABASE_URL).
const REPO_ROOT = path.resolve(process.cwd(), "..", "..");

// `tsx` is a devDependency of the workspace root, so pnpm links it into the
// root .bin even though node_modules is not hoisted. If that ever stops being
// true this must fail loudly rather than spawn nothing.
const TSX_BIN = path.join(REPO_ROOT, "node_modules", ".bin", "tsx");
const CLI_ENTRY = path.join("apps", "cli", "src", "cli.ts");
...
export async function startJob(kind: JobKind, opts: JobOptions): Promise<StartResult> {
  ...
  try {
    if (!existsSync(TSX_BIN)) {
      throw new Error(
        `tsx not found at ${TSX_BIN}. Run \`pnpm install\` at the workspace root.`
      );
    }
