import type { BudgetPort, HttpPort, PlaceDetails, PlaceRef, PlacesPort } from "../ports/index";
...
export const MASKS = {
  ...
  searchPro:
    "places.id,places.displayName,places.formattedAddress,places.primaryType,places.businessStatus,nextPageToken",
  detailsEnterprise:
...
export function createGooglePlaces(opts: { apiKey: string; http?: HttpPort }): PlacesPort {
   ...
   */
  async function* searchIds(query: string, budget: BudgetPort): AsyncGenerator<PlaceRef[]> {
  ...
  async function getDetails(placeId: string, budget: BudgetPort): Promise<PlaceDetails> {
  ...
  async function refreshPlaceId(placeId: string): Promise<string | null> {
    const res = await fetchWithRetry(
      `${BASE_URL}/places/${placeId}`,
      { headers: { "X-Goog-Api-Key": opts.apiKey, "X-Goog-FieldMask": "id" } },
      "Place ID refresh"
    );
  ...
  return { searchIds, getDetails, refreshPlaceId };
}
