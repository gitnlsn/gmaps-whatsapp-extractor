import type { BudgetPort, Db } from "../ports/index";
...
/**
 * Month-to-date usage as data. Formatting lives in the app — the core reports,
 * it does not print.
 */
export async function usageReport(db: Db): Promise<{ rows: SkuUsage[]; overage: number }> {
  const raw = await db.query<{ sku: string; total: string }>(
  ...
  const rows = raw.map((r) => {
    ...
    const cost = (paid / 1000) * (PRICE_PER_1K[r.sku] ?? 0);
    overage += cost;
    return { sku: r.sku, used, limit, paid, cost };
  });
