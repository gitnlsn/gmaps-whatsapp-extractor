...
import type { OfferSpec } from "../domain/spec";
...
/**
 * Who the message is from. Passed in rather than read from the environment:
 * identifying the sender in the first line is an LGPD safeguard (LIA.md §5),
 * and a safeguard that silently reads a blank env var is not a safeguard.
 */
...
export async function draftMessage(deps: Deps, input: DraftInput, s: Sender): Promise<string> {
  ...
  if (!deps.llm || !input.hook) {
    return templateDraft(input, s);
  }

  try {
    const { value } = await deps.llm.completeJson<{ message: string }>({
      task: "draft",
      schema: SCHEMA,
