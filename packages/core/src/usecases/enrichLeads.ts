...
export async function checkSite(
  ...
  try {
    const res = await requireHttp(deps).fetch(url, {
      redirect: "follow",
...
export async function pageSpeed(deps: Deps, url: string, apiKey?: string): Promise<number | null> {
  const params = new URLSearchParams({ url, strategy: "mobile", category: "performance" });
  if (apiKey) params.set("key", apiKey);

  try {
    const res = await requireHttp(deps).fetch(
...
export interface EnrichOptions {
  ...
  recheck: boolean;
  offerId?: string;
  /** Optional PageSpeed key. The free tier needs none; a key raises the quota. */
  psiApiKey?: string;
}
...
export async function enrichLeads(deps: Deps, opts: EnrichOptions): Promise<EnrichResult> {
  ...
  const rows = await deps.db.query<{ cnpj: string; website: string | null; email: string | null }>(
  ...
  if (rows.length === 0) {
    deps.progress.finish("enrich", "nada a enriquecer (leads precisam de telefone)");
    return { checked: 0, noSite: 0, dead: 0, hub: 0, ok: 0 };
  }

  deps.progress.stage("enrich", `Enriquecendo sites (concorrência ${opts.concurrency})`, rows.length);
  let done = 0;
  let noSite = 0;
  let dead = 0;
  let hub = 0;
  ...
  await mapLimit(rows, opts.concurrency, async (row) => {
    ...
    const signals = await checkSite(deps, candidate);

    if (opts.psi && signals.finalUrl && !signals.isDead && !signals.isLinkHub) {
      signals.psiPerformance = await pageSpeed(deps, signals.finalUrl, opts.psiApiKey);
    }
    ...
    await upsertEnrichment(deps, row.cnpj, signals, signalsJson);
    ...
    if (done % 25 === 0 || done === rows.length) {
      deps.progress.tick(done);
    }
  });

  const result = { checked: rows.length, noSite, dead, hub, ok: rows.length - noSite - dead - hub };
  deps.progress.finish(
    "enrich",
    `sem site ${noSite}, morto ${dead}, link-hub ${hub}, ok ${result.ok}`
  );
  return result;
}
