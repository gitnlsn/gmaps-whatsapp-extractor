...
import { classifyReceitaPhone } from "../domain/phone";
...
export async function latestPeriod(deps: Deps): Promise<string> {
  const res = await requireHttp(deps).fetch(`${RF_BASE}/`);
  if (!res.ok) {
    throw new Error(
...
async function download(deps: Deps, url: string, dest: string): Promise<void> {
  const existing = await fileSize(dest);

  const head = await requireHttp(deps).fetch(url, { method: "HEAD" });
  if (!head.ok) throw new Error(`HEAD ${url} failed (${head.status})`);
  const total = Number(head.headers.get("content-length") ?? 0);

  if (existing !== undefined && total > 0 && existing === total) {
    deps.progress.info(`  ${dest.split("/").pop()} already complete (${fmtBytes(total)}).`);
    return;
  }
  ...
  if (existing !== undefined && existing > 0 && existing < total) {
    ...
    mode = "a";
    start = existing;
    deps.progress.info(`  Resuming at ${fmtBytes(existing)} / ${fmtBytes(total)}...`);
  }

  const res = await requireHttp(deps).fetch(url, { headers });
  ...
  const meter = new Transform({
    transform(chunk, _enc, cb) {
      ...
      if (Date.now() - lastLog > 2000) {
        ...
        deps.progress.tick(received, `${fmtBytes(received)} / ${fmtBytes(total)} (${pct}%)`);
        lastLog = Date.now();
      }
      cb(null, chunk);
    },
  ...
  deps.progress.tick(received, `${fmtBytes(received)} / ${fmtBytes(total)} (100%)`);
}
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      lines.on("data", (line: string) => {
        ...
        if (stats.read % 1_000_000 === 0) {
          deps.progress.tick(stats.read, `kept ${stats.kept.toLocaleString()}`);
        }
      });
 ...
 */
export async function loadCnaes(deps: Deps): Promise<void> {
  const period = await latestPeriod(deps);
  const zip = join(DATA_DIR, "Cnaes.zip");

  deps.progress.info(`Receita Federal period: ${period}`);
  await download(deps, `${RF_BASE}/${period}/Cnaes.zip`, zip);

  await deps.db.withClient((c) => c.query("TRUNCATE cnaes"));
  ...
  const rows = await deps.db.query<{ codigo: string; descricao: string; n: string }>(
...
export async function loadReceita(deps: Deps, opts: LoadOptions): Promise<void> {
  const period = opts.period ?? (await latestPeriod(deps));
  const parts = opts.parts ?? [0];
  const ufs = opts.uf?.map((u) => u.toUpperCase());
  const cnaes = opts.cnae;

  deps.progress.info(`Receita Federal period: ${period}`);
  deps.progress.info(`Parts: ${parts.join(", ")}  (0-9 available; each ~1/10 of the base)`);
  deps.progress.info(`UF filter: ${ufs?.join(",") ?? "all"}`);
  deps.progress.info(`CNAE prefix filter: ${cnaes?.join(",") ?? "all"}`);
  ...
  if (opts.dryRun) {
    ...
    for (const p of parts) {
      for (const name of [`Estabelecimentos${p}.zip`, `Empresas${p}.zip`]) {
        ...
        deps.progress.info(`  ${head.ok ? "OK " : "MISS"}  ${name}  ${fmtBytes(size)}`);
      }
    }
  ...
  await deps.db.withClient((c) => c.query(STAGING_DDL));
  ...
  if (opts.normalizeOnly) {
    const [{ n }] = await deps.db.query<{ n: string }>(
    ...
    await normalize(deps, period);
    await reportFillRates(deps);
    return;
  }
  ...
  deps.progress.info("\n[1/5] Municípios (reference table)");
  {
    ...
    await download(deps, `${RF_BASE}/${period}/Municipios.zip`, zip);
    const stats: FilterStats = { read: 0, kept: 0, withPhone: 0, withMobileShape: 0 };
    await copyFiltered(
      deps,
      zip,
    ...
    deps.progress.info(`  ${stats.kept.toLocaleString()} municípios.`);
  }
  ...
  for (const p of parts) {
    ...
    deps.progress.info(`  Estabelecimentos${p}.zip`);
    await download(deps, `${RF_BASE}/${period}/Estabelecimentos${p}.zip`, zip);
  ...
  if (ufs || cnaes) {
    const rows = await deps.db.query<{ cnpj_basico: string }>(
      "SELECT DISTINCT cnpj_basico FROM rf_estab_stage"
    );
    if (rows.length <= MAX_SET) {
      wanted = new Set(rows.map((r) => r.cnpj_basico));
      deps.progress.info(`  ${wanted.size.toLocaleString()} distinct company roots to join.`);
    }
  }
  ...
  for (const p of parts) {
    ...
    deps.progress.info(`  Empresas${p}.zip`);
    await download(deps, `${RF_BASE}/${period}/Empresas${p}.zip`, zip);
    const stats: FilterStats = { read: 0, kept: 0, withPhone: 0, withMobileShape: 0 };
    await copyFiltered(
      deps,
      zip,
  ...
  deps.progress.info("\n[4/5] Simples / MEI");
  {
    ...
    await download(deps, `${RF_BASE}/${period}/Simples.zip`, zip);
    const stats: FilterStats = { read: 0, kept: 0, withPhone: 0, withMobileShape: 0 };
    await copyFiltered(
      deps,
      zip,
...
async function reportFillRates(deps: Deps): Promise<void> {
  const [fill] = await deps.db.query<{
    total: string;
    with_phone: string;
    mobile: string;
    with_email: string;
  ...
  deps.progress.info(`  leads:        ${Number(fill.total).toLocaleString()}`);
  deps.progress.info(
...
async function normalize(deps: Deps, period: string): Promise<void> {
  ...
  deps.progress.info("  Resolving remaining município names by similarity...");
  await deps.db.withClient((c) =>
    c.query(`
      UPDATE leads l
  ...
  deps.progress.info("  Classifying phone numbers...");
  await classifyPending(deps);
}
 ...
 */
export async function classifyPending(deps: Deps, batchSize = 20000): Promise<void> {
  let processed = 0;
  let mobiles = 0;

  for (;;) {
    const rows = await deps.db.query<{ cnpj: string; phone_ddd: string | null; phone_local: string | null }>(
    ...
    deps.progress.tick(processed, `${mobiles.toLocaleString()} mobile`);
  }
