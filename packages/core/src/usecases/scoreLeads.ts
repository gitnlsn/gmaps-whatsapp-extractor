...
import { bestFit, tierFor, type OfferSpec } from "../domain/spec";
import { resolveOffer, type LoadedOffer } from "./offerRepo";
import { buildStage0Where } from "../domain/rank";
...
export interface ScoreOptions {
  ...
  rescore: boolean;
  offerId?: string;
  /** Daily free-tier request ceiling. Resolved by the app, not read here. */
...
export async function scoreLeads(deps: Deps, opts: ScoreOptions): Promise<ScoreRunResult> {
  ...
  if (candidates.length === 0) {
    progress.finish(
      "score",
      `nada a pontuar em "${offer.id}" — rode o enrich antes, ou amplie o alvo da oferta`
    );
    return { scored: 0, failed: 0, tiers: { hot: 0, warm: 0, cold: 0 } };
  }

  const model = llm.modelFor("score");
  ...
  const llmBudget = createLlmBudget(deps.db, { dailyRequests: opts.llmDailyRequests });
  const quota = await llmBudget.usageReport();
  await llmBudget.checkBudget(model, requests);

  progress.stage("score", "Pontuando leads", candidates.length);
  progress.info(
    `Offer: ${offer.title} (${offer.id} v${offer.version}) — rubrica ${sha}\n` +
      ...
      `Cota do dia: ${quota.used}/${quota.limit} usadas, ${quota.left} restantes. ` +
  ...
  for (let i = 0; i < candidates.length; i += opts.batchSize) {
    ...
    try {
      const { value } = await llm.completeJson<{ results?: ScoreResult[] }>({
        task: "score",
        schema,
      ...
      for (const c of batch) {
        ...
        if (!r) {
          await recordFailure(deps, c.cnpj, offer, model, "model omitted this cnpj from its response");
          failed++;
          continue;
        }
        const tier = await recordScore(deps, c.cnpj, offer, model, sha, spec, r);
        scored++;
      ...
      for (const c of batch) {
        await recordFailure(deps, c.cnpj, offer, model, msg.slice(0, 400));
        failed++;
      }
      progress.warn(`Batch failed: ${msg.slice(0, 160)}`);
    }

    progress.tick(
      Math.min(i + opts.batchSize, candidates.length),
  ...
  progress.finish(
    "score",
    `hot ${tiers.hot ?? 0}, warm ${tiers.warm ?? 0}, cold ${tiers.cold ?? 0}, failed ${failed}` +
      (failed > 0 ? " — falhas ficam com score NULL e o erro gravado, nunca um 5 falso" : "")
  );

  return { scored, failed, tiers };
}
