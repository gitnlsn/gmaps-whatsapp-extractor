...
import { buildStage0Where, buildRankSql, buildRankPartsSql } from "../domain/rank";
import type { LoadedOffer } from "./offerRepo";
...
export async function countReach(deps: Deps, offer: LoadedOffer): Promise<ReachCounts> {
  const params: unknown[] = [];
  const p = (v: unknown) => `$${params.push(v)}`;
  const where = buildStage0Where(offer.spec, p);

  const rows = await deps.db.query<Record<string, string>>(
