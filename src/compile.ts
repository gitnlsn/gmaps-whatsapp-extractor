...
import { completeJson, modelFor, hasApiKey } from "./llm";
import { parseOfferSpec, type OfferSpec } from "./offers/spec";
...
export async function compileOffer(description: string): Promise<CompileResult> {
  if (!hasApiKey()) {
    throw new Error(
  ...
  const model = modelFor("compile");

  const { value: t } = await completeJson<Record<string, any>>({
    task: "compile",
  ...
  const { value: r } = await completeJson<Record<string, any>>({
    task: "compile",
