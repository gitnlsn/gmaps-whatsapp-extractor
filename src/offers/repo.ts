...
import { parseOfferSpec, type OfferSpec } from "./spec";
import { LEGACY_OFFER_ID, LEGACY_SPEC } from "./legacy";
...
export async function loadOffer(id: string): Promise<LoadedOffer> {
  ...
  const row = rows[0];
  if (!row) {
    const exists = await query<{ id: string }>(`SELECT id FROM offers WHERE id = $1`, [id]);
    if (exists.length) {
      throw new Error(
        `Offer "${id}" has no spec for version ${exists[0] ? "current" : "?"}. ` +
          `If this is the original offer, run: npm start -- offer seed`
      );
    }
    throw new Error(`No offer named "${id}". Run \`npm start -- offer list\` to see them.`);
  }
...
export async function activeOffer(): Promise<LoadedOffer> {
  const rows = await query<{ id: string }>(`SELECT id FROM offers WHERE active LIMIT 1`);
  if (!rows[0]) {
    throw new Error(
      "No active offer. Activate one with `npm start -- offer use <id>`, " +
        "or create one with `npm start -- offer compile`."
    );
  }
  return loadOffer(rows[0].id);
}

/** Resolves an optional --offer flag, falling back to the active offer. */
export async function resolveOffer(id?: string): Promise<LoadedOffer> {
  return id ? loadOffer(id) : activeOffer();
}
...
export async function saveSpec(input: SaveSpecInput): Promise<number> {
 ...
 * spec is large and lives in TypeScript (src/offers/legacy.ts), and duplicating
...
export async function seedLegacy(): Promise<void> {
  ...
  if (existing.length) {
    console.log(`Offer "${LEGACY_OFFER_ID}" already has spec v${existing[0].version}. Nothing to do.`);
    return;
  }
  ...
  console.log(`Seeded "${LEGACY_OFFER_ID}" spec v${version}.`);
}
