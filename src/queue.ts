...
export async function reviewQueue(opts: QueueOptions): Promise<void> {
  ...
  try {
    for (const [i, row] of rows.entries()) {
      ...
      if (answer === "n") {
        await mark(row.cnpj, "not_a_fit", draft, offer.id);
        continue;
      }

      if (answer === "o") {
        await mark(row.cnpj, "opted_out", draft, offer.id);
        await withClient((c) =>
          c.query(
      ...
      if (answer === "i") {
        await recordInterest(row.cnpj, offer.id, rl);
        continue;
      }

      if (answer === "s") {
        await mark(row.cnpj, "sent", draft, offer.id);
        sent++;
        if (already + sent >= cap) {
