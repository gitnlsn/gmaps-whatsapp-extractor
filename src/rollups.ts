 ...
 */
export async function refreshRollups(opts: { quiet?: boolean } = {}): Promise<void> {
  for (const name of ROLLUPS) {
    ...
    if (!opts.quiet) {
      console.log(`  refreshed ${name} (${Date.now() - started} ms)`);
    }
  }
}
 ...
 */
export async function refreshRollupsQuietly(): Promise<void> {
  try {
    await refreshRollups({ quiet: true });
  } catch (err) {
    console.warn(`  (rollup refresh failed: ${(err as Error).message})`);
  }
}
